...
function Sidebar() {
  ...
  const items = [
    ...
    { path: '/staff/grades', label: 'Enter Grades', icon: '🎓' },
...
function DashboardHome() {
  const [stats, setStats] = useState({ students: 0, documents: 0, missing: 0 });
  const [recent, setRecent] = useState([]);

  useEffect(() => {
    // TODO: replace with real endpoint when backend ready
    setStats({ students: 1240, documents: 8520, missing: 37 });
    setRecent([
      { id: 1, action: 'Student registered', user: 'records01', time: '2 mins ago' },
      { id: 2, action: 'Document uploaded', user: 'records01', time: '15 mins ago' },
      { id: 3, action: 'Grade recorded', user: 'lecturer_john', time: '1 hour ago' },
    ]);
  }, []);
    ...
    <div>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="value" style={{ color: 'var(--cul-green)' }}>{stats.students.toLocaleString()}</div>
        </div>
        <div className="stat-card">
          <h3>Documents Stored</h3>
          <div className="value" style={{ color: 'var(--cul-blue)' }}>{stats.documents.toLocaleString()}</div>
        </div>
        <div className="stat-card">
          <h3>Missing Documents</h3>
          <div className="value" style={{ color: 'var(--cul-danger)' }}>{stats.missing}</div>
        </div>
        <div className="stat-card">
          <h3>Colleges</h3>
            ...
            <tbody>
              {recent.map((r) => (
                <tr key={r.id}>
                  <td>{r.action}</td>
                  <td>{r.user}</td>
                  <td style={{ color: 'var(--cul-gray-400)' }}>{r.time}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function GradesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess('');
    try {
      await api.createGrade({ student_id: parseInt(studentId), course_id: parseInt(courseId), grade, session, semester });
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Enter Grades</h1>
        <p>Record academic grades for students</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ maxWidth: '600px' }}>
        <div className="form-group">
          <label className="form-label">Student ID</label>
          <input type="number" className="form-input" value={studentId} onChange={(e) => setStudentId(e.target.value)} required />
        </div>

        <div className="form-group">
          <label className="form-label">Course ID</label>
          <input type="number" className="form-input" value={courseId} onChange={(e) => setCourseId(e.target.value)} required />
        </div>
        ...
        <div className="form-group">
          <label className="form-label">Academic Session (e.g. 2024/2025)</label>
          <input className="form-input" value={session} onChange={(e) => setSession(e.target.value)} required placeholder="2024/2025" />
        </div>
